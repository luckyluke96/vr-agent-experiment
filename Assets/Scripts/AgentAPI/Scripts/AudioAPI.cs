...
public class AudioAPI : MonoBehaviour
{
    ...
    public bool ReadyToReceiveAudio
    {
        get
        {
            return clientWebSocket != null && clientWebSocket.State == WebSocketState.Open && AudioModeSet;
        }
    }

    public void StartSpeechToTextAPI(Action Connected_CB, Action<TranscriptionResult> IncomingTranscription_CB)
    {
        ConnectToWebsocket(Connected_CB, IncomingTranscription_CB);
    }

    private void ConnectToWebsocket(Action Connected_CB, Action<TranscriptionResult> IncomingTranscription_CB)
    {
        ...
        {
            var connectingTask = clientWebSocket.ConnectAsync(new System.Uri(AgentSettings.sst_server), token);
                    ...
                    if (result.Count > 0)
                    {
                        if (result.MessageType == WebSocketMessageType.Text && result.EndOfMessage)
                        {
                            ...
                            if (text.StartsWith("{"))
                            {
                                ...
                                var deserialized = JsonConvert.DeserializeObject<WebsocketUTF8Message<object>>(text);
                                if (deserialized.message == "stt")
                                {
                                    var temp = JsonConvert.DeserializeObject<WebsocketUTF8Message<TranscriptionResult>>(text, new JsonSerializerSettings
                                    {
                                    ...
                                    IncomingTranscription_CB.Invoke(lastTranscription);
                                }else if (deserialized.message == "chat")
                                {
                                    var temp = JsonConvert.DeserializeObject<WebsocketUTF8Message<ChatResult>>(text);
                                ...
                                {
                                    Debug.Log("Message: " + result.MessageType + " " + result.EndOfMessage + " " + result.Count + " " + text);
                                }
            ...
            var task = clientWebSocket.SendAsync(GetUTF8BytesForString("{message: \"Hello\"}"), WebSocketMessageType.Text, true, token);
            task.Wait();
    ...
    internal bool sendAudioData(ArraySegment<byte> arr)
    {
        ...
        while (l < arr.Count)
        {
            ...
            clientWebSocket.SendAsync(arr.Slice(l, length), WebSocketMessageType.Binary, l + length >= arr.Count, token);
            l += length;
        }

        //arr.Slice(0, 100);
        //clientWebSocket.SendAsync(arr.Slice(0, 64000), WebSocketMessageType.Binary, true, token);
    ...
    public Task EnableAudioWebsocket(AudioConfiguration config, Action<bool> SpeechRecognitionReady_Callback)
    {
        ...
        {
            await clientWebSocket.SendAsync(new WebsocketUTF8Message<AudioConfiguration>("start-audio", config).GetPackage(), WebSocketMessageType.Text, true, token);
            AudioModeSet = true;
    ...
    public Task SendChatGPTRequest(Dictionary<string, string> request)
    {
        if (clientWebSocket == null || clientWebSocket.State != WebSocketState.Open)
        {
            // Connection not available
            Debug.LogError("Connection to Websocket not available");
            return null;
        }

        Task sendChatGPTRequest = new Task(async () =>
        {
            await clientWebSocket.SendAsync(new WebsocketUTF8Message<Dictionary<string, string>>("chat-gpt", request).GetPackage(), WebSocketMessageType.Text, true, token);
        });
        sendChatGPTRequest.Start();
        return sendChatGPTRequest;
    }
    ...
    public Task StopSTT_Task()
    {
        ...
        {
            await clientWebSocket.SendAsync(new WebsocketUTF8Message<string>("stop-audio", "").GetPackage(), WebSocketMessageType.Text, true, token);
            AudioModeSet = false;
        });
        stopAudio.Start();
        return stopAudio;
