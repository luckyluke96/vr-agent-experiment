...
public class SimpleSpectrum : MonoBehaviour
{
    ...
    */
    ...
    /// </summary>
    [Tooltip(
        "The amount of bars to use. Does not have to be equal to Num Samples, but probably should be lower."
    )]
    ...
    [Range(0, 1)]
    [Tooltip(
        "The amount of dampening used when the new scale is lower than the bar's existing scale."
    )]
    ...
    /// </summary>
    [Tooltip(
        "Determines whether to apply a color gradient on the bars, or just use a solid color."
    )]
    ...
    public AnimationCurve colorValueCurve = new AnimationCurve(
    ...
    [Range(0, 1)]
    [Tooltip(
        "The amount of dampening used when the new color value is higher than the existing color value."
    )]
    ...
    [Range(0, 1)]
    [Tooltip(
        "The amount of dampening used when the new color value is lower than the existing color value."
    )]
        ...
        for (int i = 0; i < barAmount; i++)
        {
            ...
            if (barCurveAngle > 0) //apply spectrum bending
            {
                ...
                barClone.transform.localPosition =
                    new Vector3(
                        Mathf.Sin(thisBarAngleR) * curveRadius,
                        0,
                        Mathf.Cos(thisBarAngleR) * curveRadius
                    ) + curveCentreVector;
                barClone.transform.localRotation = Quaternion.Euler(barXRotation, thisBarAngleD, 0);
            }
            else //standard positioning
            {
                barClone.transform.localPosition = new Vector3(
                    i * (1 + barXSpacing) - midPoint,
                    0,
                    0
                );
            }
    ...
    public void RestartMicrophone()
    {
        ...
        if (sourceType == SourceType.MicrophoneInput || sourceType == SourceType.StereoMix)
        {
            ...
            {
                Debug.LogError(
                    "Error from SimpleSpectrum: Microphone or Stereo Mix is being used, but no Microphones are found!"
                );
            }
            ...
            if (sourceType == SourceType.StereoMix) //find stereo mix
            {
                ...
                if (microphoneName == null)
                    Debug.LogError(
                        "Error from SimpleSpectrum: Stereo Mix not found. Reverting to default microphone."
                    );
            }
...
#else
        if (
            sourceType == SourceType.MicrophoneInput
            || sourceType == SourceType.StereoMix
            || sourceType == SourceType.AudioSource
        )
        {
            Debug.LogError(
                "Error from SimpleSpectrum: Microphone, Stereo Mix or AudioSource cannot be used in WebGL!"
            );
        }
#endif
    }
    ...
    void Update()
    {
        if (isEnabled)
        {
            ...
            for (int i = 0; i < bars.Length; i++)
            {
                ...
                if (useLogarithmicFrequency)
                {
                    ...
                    trueSampleIndex =
                        Mathf.Lerp(
                            frequencyLimitLow,
                            freqLim,
                            (highestLogFreq - Mathf.Log(barAmount + 1 - i, 2)) / highestLogFreq
                ...
                else
                {
                    ...
                    trueSampleIndex =
                        Mathf.Lerp(frequencyLimitLow, freqLim, ((float)i) / barAmount)
                        * frequencyScaleFactor;
                ...
                value = Mathf.SmoothStep(
                    spectrum[sampleIndexFloor],
                    spectrum[sampleIndexFloor + 1],
                ...
                float oldYScale = oldYScales[i],
                    newYScale;
                if (value * barYScale > oldYScale)
                {
                    newYScale = Mathf.Lerp(
                        oldYScale,
                        Mathf.Max(value * barYScale, barMinYScale),
                        attackDamp
                    );
                }
                else
        ...
        { //switched off
            ...
            {
                bar.localScale = Vector3.Lerp(
                    bar.localScale,
                    new Vector3(1, barMinYScale, 1),
                    decayDamp
                );
            }
        }
    ...
    public static float[] GetLogarithmicSpectrumData(
        ...
        int spectrumSize,
        int sampleSize,
        FFTWindow windowUsed = FFTWindow.BlackmanHarris,
        int channelUsed = 0
    )
    {
#if UNITY_WEBGL
        ...
        {
            float trueSampleIndex =
                (highestLogSampleFreq - Mathf.Log(spectrum.Length + 1 - i, 2))
